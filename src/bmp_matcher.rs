...
{
    fn not_found_warning(filtered_out: &mut Vec<DeviceInfo>){
        match filtered_out.len() {
            0 => {
                return
            },
            1 => {
                let device_info = filtered_out.pop().expect("Length > 0 should return an valid pop.");
                if let Ok(bmpdev) = BmpDevice::from_usb_device(device_info) {
                    warn!("Matching device not found, but and the following Black Magic Probe device was filtered out: {}",
                        &bmpdev,
                    );
                } else {
            ...
            count => {
                warn!("Matching devices not found but '{}' Black Magic Probes devices were filtered out.",
                    count,
                );
            }
        }
    ...
    pub fn pop_all(&mut self) -> color_eyre::Result<Vec<BmpDevice>>
    {
        if self.found.is_empty() {
            ...
            Self::not_found_warning(&mut self.filtered_out);
